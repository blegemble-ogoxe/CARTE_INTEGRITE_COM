...
static const char                       AudioFilePathDefault[] = "/tmp/sdcard/alerte.wav";
static char                             AudioFilePath[] = "/tmp/sdcard/alerte.wav";
...
COMPONENT_INIT
{
    ...
    FILE* config;   
    char str_cfg[3]={0,0,0}; 
    char* alert_level = str_cfg;
    
    le_result_t  res;

  // res = le_appCtrl_Start("voiceCallService");
...
//Récupération du numéro de config (pré-alerte ou alerte)    

    config = fopen("/tmp/sdcard/cfg.txt","r");   
    while (fgets(str_cfg,2,config)!=NULL){
    	strncpy (alert_level,str_cfg,1); 
    	LE_INFO("NUMERO CONFIG1 : %c(%x)",str_cfg[0],str_cfg[0]);
    	puts(alert_level);
    }
    fclose(config);
    
//lecture des numéros de téléphones et des paramètres associés

 	FILE* fichier_numeros;
 	LE_INFO("3str_cfg[0]= %x",str_cfg[0]);   
	char numero[12]; 
	char alert1; 
	char alert2;    
	char* text = "Alerte Ogoxe niveau1!";
	char* text2 = "Alerte Ogoxe niveau2!";
	//char* number = str;
	int return_value=3;
	fichier_numeros = fopen("/tmp/sdcard/numeros.txt", "r");
	LE_INFO("ouverture fichier numero");
    	int niveau_alerte = 0;
    	niveau_alerte = str_cfg[0];
	while (return_value!=EOF){    
		return_value=fscanf(fichier_numeros," %s %c %c",numero, &alert1, &alert2);
		LE_INFO("numero tel : %s",numero);
		LE_INFO("return value : %d",return_value);
		if(return_value==3){
		LE_INFO("return value == 3");
			if(niveau_alerte == 0x31)//pré-alerte
			//if(*alert_level == 1)//pré-alerte
	     	   	{ 
	     	   	LE_INFO("str_cfg[0]==1 !");
	     	   		if(alert1=='S') {//l'utilisateur demande de recevoir un SMS
	     	   		LE_INFO("alert1=='S'");
	     	   		LE_INFO("sending sms...");
	     	   			res = smsmo_SendMessage(numero, text);
	     	   			 if (res != LE_OK){
	     	   		...
	     	   		else if (alert1=='A'){//l'utilisateur demande de recevoir un appel
	     	   		LE_INFO("alert1=='A'");
	     	   		LE_INFO("making call...");
	     	   			VoiceCallHandlerRef = le_voicecall_AddStateHandler(MyCallEventHandler, NULL);
					myCallRef = le_voicecall_Start(numero);
					if (!myCallRef)
					{
					...
					VoiceCallHandlerRef = le_voicecall_AddStateHandler(MyCallEventHandler, NULL);
					ctrlVC_HoldCall();
					OpenAudioFile(myCallRef);
					VoiceCallHandlerRef = le_voicecall_AddStateHandler(MyCallEventHandler, NULL);
	     	   		}
	     	   	}
	     	   	else if (niveau_alerte=='2'){//alerte
	     	   	LE_INFO("str_cfg[0]=='2'");
	     	   		LE_INFO("sending sms...");
	     	   		if(alert2=='S') {
	     	   		LE_INFO("alert2=='S'");
	     	   		LE_INFO("sending sms...");
	     	   			res = smsmo_SendMessage(numero, text2);
	     	   			 if (res != LE_OK){//l'utilisateur demande de recevoir un SMS
	     		   			LE_ERROR("SmsMoMessage has failed (res.%d)!", res);
	     	   			}
	     	   			else
	     	  			{
	     	   		...
	     	   		else if (alert2=='A'){//l'utilisateur demande de recevoir un appel
	     	   		LE_INFO("alert2=='A'");
	     	   		LE_INFO("making call...");
	     	   			VoiceCallHandlerRef = le_voicecall_AddStateHandler(MyCallEventHandler, NULL);
					myCallRef = le_voicecall_Start(numero);
        ...
        fclose(fichier_numeros);
    
   /* 
    while( fgets (str, 20, demo)!=NULL )
     {
     	   strncpy (number, str, 12);
     	   //str2[12]= '\0';
     	   puts(number);
     	   LE_INFO("STOP");
     	   if(str_cfg[0] == '1')
     	   { 
     	   	res = smsmo_SendMessage(number, text);
     	   }
     	   else if(str_cfg[0] == '2')
     	   { 
     	   	res = smsmo_SendMessage(number, text2);
     	   }
     	   
     	   else LE_ERROR("Couldn't read config number") ;
     ...
     // closes the file pointed by fichier_numeros*/
